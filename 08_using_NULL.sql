...
-- 2. Note the INNER JOIN misses the teachers with no department and the departments with 
--    no teacher.
...
SELECT dept.name, COUNT(teacher.name) AS total FROM
...
-- 10. Use CASE to show the name of each teacher followed by 'Sci' if the teacher is in dept 1 or 2, show 
